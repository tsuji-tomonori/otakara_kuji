...
ApiBodySelf = TypeVar("ApiBodySelf", bound="ApiBody")
...
class ApiBody(NamedTuple):
    ...
    def to_dynamo_items(self: ApiBodySelf) -> list[dict]:
        return [
            d | {"category": self.category, "item_id": i}
...
def service(
    ...
    if response_category is not None:
        raise ClientError(
            ...
            message=f"category is already exists: {body.category}",
        )
    ...
    put_items(
        db_resource=db_resource,
        table_name=env.CATEGORY_TABLE_NAME,
        items=[{"category": body.category, "num_item": len(body.items)}],
    )
    return Response(
        status_code=200,
        message="success",
